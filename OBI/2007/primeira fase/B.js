const [downloaded, available] = prompt('apps baixados e disponíveis')
.split(' ')
.map(Number);
...
for (let i = 0; i < downloaded; i++) {
    const [app, version] = prompt('digite o app baixado e a versão')
    .split(' ');
    downloadeds.push({
        app: String(app),
...
for (let i = 0; i < available; i++) {
    const [app, version] = prompt('digite o app disponível e a versão')
    .split(' ');
    availables.push({
        app: String(app),
        version: Number(version)
