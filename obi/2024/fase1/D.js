...
function getNextState(game = [[]]) {
    const nextStateGame = game.map(row => [...row]);
...
export default function simulateGameOfLife(game, steps) {
    let currentGame = game.map(row => [...row]);

    for (let i = 0; i < steps; i++) {
        currentGame = getNextState(currentGame);
    }
